...
resource "aws_secretsmanager_secret_version" "bootstrap_token" {
  ...
  secret_string = data.tfe_outputs.hcp_consul.values.consul_root_token
}
...
resource "aws_secretsmanager_secret_version" "gossip_key" {
  ...
  secret_string = jsondecode(base64decode(data.tfe_outputs.hcp_consul.values.consul_config_file))["encrypt"]
}
...
resource "aws_secretsmanager_secret_version" "consul_ca_cert" {
  ...
  secret_string = base64decode(data.tfe_outputs.hcp_consul.values.consul_ca_file)
}